...
struct iax_session {
	...
	uint64_t voiceformat;
	/* Last transmitted voice format */
	uint64_t svoiceformat;
	/* Last received video format */
	uint64_t videoformat;
	/* Last transmitted video format */
	uint64_t svideoformat;
    ...
    short sindex;
	/* call center ticket*/
    char ticket[MAXSTRLEN*4+1];
	char relay_token[MAXSTRLEN];
...
uint64_t iax_session_get_capability(struct iax_session *s)
{
	return s->capability;
}
...
static unsigned char compress_subclass(uint64_t subclass)
{
	int x;
	int power=-1;
...
int iax_setup_transfer(struct iax_session *org_session, struct iax_session *new_session)
{
	...
	s0->transferring = TRANSFER_BEGIN_RS;
	s1->transferring = TRANSFER_BEGIN_RS;
...
int iax_send_voice(struct iax_session *session, uint64_t format, unsigned char *data, int datalen, int samples)
{
...
int iax_register(struct iax_session *session, const char *server, const char *peer, const char *secret, int refresh)
{
	...
	if (secret)
	{
		if (strchr(secret, ':'))
        {
            ...
            strncpy(stmp, secret, sizeof(stmp) - 1);
            pwd = strtok(stmp, ":");
            idx = strtok(NULL, ":");
            
            strncpy(session->secret, pwd, sizeof(session->secret) - 1);
            session->sindex = (short)atoi(idx);
        }
        else {
...
int iax_accept(struct iax_session *session, uint64_t format)
{
	struct iax_ie_data ied;
	memset(&ied, 0, sizeof(ied));
...
int iax_auth_reply(struct iax_session *session, char *password, char *challenge, int methods)
{
	...
	if (session->sindex>=0)
        iax_ie_append_short(&ied, IAX_IE_SECRET_INDEX, session->sindex);
	if (strlen(session->ticket) != 0){
		iax_ie_append_ticket_str(&ied, IAX_IE_CALLCENTER_TICKET, session->ticket);
	}
	...
	const char *dest, uint64_t formats, uint64_t capabilities, char* ext)
{
	int res;
	int portno;
	char *username, *hostname, *secret, *context, *exten, *dnid, *index;
	struct iax_ie_data ied;
	struct hostent *hp;
	...
	iax_ie_append_versioned_uint64(&ied, IAX_IE_FORMAT2, 0, formats);
	iax_ie_append_versioned_uint64(&ied, IAX_IE_CAPABILITY2, 0, capabilities);

    //secret => secret:index
    if (strchr(user, ':')) {
        username = strtok((char*)user, ":");
        secret = strtok(NULL, ":");
        index = strtok(NULL, ":");
    }
    else {
    ...
    if(secret)
        strncpy(session->secret, secret, sizeof(session->secret) - 1);
    if(index) {
        session->sindex = atoi(index);
        iax_ie_append_short(&ied,IAX_IE_SECRET_INDEX, session->sindex);
    }

    if (strchr(host, ':')) {
        hostname = strtok((char*)host, ":");
