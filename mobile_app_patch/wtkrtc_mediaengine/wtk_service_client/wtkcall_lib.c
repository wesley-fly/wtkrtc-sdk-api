...
#include "wtkrtc_mediaengine/wtk_rtc_api/wtk_rtc_api.h"
...
void wtkcall_start_video(int is_front)//1:Front; 0:back
{
	//iaxci_usermsg(IAXC_ERROR, "wtkcall_start_video getpid = %d, pthread_self = %lu\n", getpid(),pthread_self());
	libwtk_create_capture(is_front);
...
void wtkcall_set_render(void* RemoteSurface)
{
