...
void iaxci_usermsg(int type, const char *fmt, ...)
{
	...
	e.ev.text.callNo = -1;
	va_start(args, fmt);
	vsnprintf(e.ev.text.message, IAXC_EVENT_BUFSIZ*2, fmt, args);
	va_end(args);
...
EXPORT void  wtkcall_perform_state_callback(struct wtkState* call, int callNo, int state, char* name, char* number)
{
	...
	if(new_activity != -1)  
	{
		...
		if(wtkcall_send_jni_event!=NULL)
		{
			...
			iaxci_usermsg(IAXC_STATUS, "wtkcall_send_jni_event:peerName=%s,peerNumber=%s,activity=%d,reason=%d(H:%d),duration=%d,type_sdk(jni)=%d",
...
EXPORT void  wtkcall_perform_message_callback(int callNo,char *message)
{
	...
	if(wtkcall_send_jni_event)
		wtkcall_send_jni_event(EVENT_MESSAGE, (void*)&info);
}
