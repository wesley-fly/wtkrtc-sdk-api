...
PlatformThread::PlatformThread(ThreadRunFunction func,
    ...
    : run_function_(func), priority_(priority), obj_(obj), is_sleep_(true), name_(thread_name) {
  RTC_DCHECK(func);
...
void PlatformThread::Run() {
  ...
  is_sleep_ = false;
  //RTC_LOG(LS_ERROR) << name_ << " Thread ===>>> GOTO WEAKUP";
  if (run_function_) {
...
#endif  // defined(WEBRTC_WIN)
  is_sleep_ = true;
  //RTC_LOG(LS_ERROR) << name_ << " Thread ===>>> GOTO SLEEP";
}
...
#endif
bool PlatformThread::IsSleep() const {
	return is_sleep_;
}
}  // namespace rtc
