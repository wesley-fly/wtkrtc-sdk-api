...
int32_t VideoRenderOpenGles20::Setup(int32_t width, int32_t height) {
  RTC_LOG(INFO) << __FUNCTION__ << ": width :height="<< width << "," << height;
  ...
  if (!_program) {
    RTC_LOG(LS_ERROR) << __FUNCTION__ << ": Could not create program";
    return -1;
  }
  ...
  if (positionHandle == -1) {
    RTC_LOG(LS_ERROR) << __FUNCTION__ <<": Could not get aPosition handle";
    return -1;
  }
  ...
  if (textureHandle == -1) {
    RTC_LOG(LS_ERROR) << __FUNCTION__ <<": Could not get aTextureCoord handle";
    return -1;
  }
    ...
    if (!compiled) {
      ...
      if (infoLen) {
        ...
        if (buf) {
          ...
          RTC_LOG(LS_ERROR) << __FUNCTION__ <<": Could not compile shader";
          free(buf);
        }
    ...
    if (linkStatus != GL_TRUE) {
      ...
      if (bufLength) {
        ...
        if (buf) {
          ...
          RTC_LOG(LS_ERROR) << __FUNCTION__ <<": Could not link program: %s";
          free(buf);
        }
      }
...
void VideoRenderOpenGles20::printGLString(const char *name, GLenum s) {
  ...
  RTC_LOG(INFO) << __FUNCTION__ << ":v=" << v;
}

void VideoRenderOpenGles20::checkGlError(const char* op) {
#ifdef ANDROID_LOG
  for (GLint error = glGetError(); error; error = glGetError()) {
    RTC_LOG(LS_ERROR) << __FUNCTION__ << "glError";
  }
#else
  return;
#endif
...
void VideoRenderOpenGles20::UpdateTextures(const VideoFrame& frameToRender) {
  ...
  //ConvertFromI420();GetI420 - > ToI420
  GlTexSubImage2D(width, height, frameToRender.video_frame_buffer()->GetI420()->StrideY(),
                  frameToRender.video_frame_buffer()->GetI420()->DataY());

  glActiveTexture(GL_TEXTURE1);
  glBindTexture(GL_TEXTURE_2D, _textureIds[1]);
  GlTexSubImage2D(width / 2, height / 2, frameToRender.video_frame_buffer()->GetI420()->StrideU(),
                  frameToRender.video_frame_buffer()->GetI420()->DataU());

  glActiveTexture(GL_TEXTURE2);
  glBindTexture(GL_TEXTURE_2D, _textureIds[2]);
  GlTexSubImage2D(width / 2, height / 2, frameToRender.video_frame_buffer()->GetI420()->StrideV(),
                  frameToRender.video_frame_buffer()->GetI420()->DataV());
