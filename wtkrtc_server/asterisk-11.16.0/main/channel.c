...
int ast_write(struct ast_channel *chan, struct ast_frame *fr)
{
	...
	switch (fr->frametype) {
	case AST_FRAME_CONTROL:
		if(fr->subclass.integer == AST_CONTROL_VIDUPDATE)
		{
			ast_log(LOG_WARNING, "res_rtp_asterisk, debug\n");
		res = (ast_channel_tech(chan)->indicate == NULL) ? 0 :
			ast_channel_tech(chan)->indicate(chan, fr->subclass.integer, fr->data.ptr, fr->datalen);
		}
		break;
...
int ast_get_ptt_response(struct ptt_request *request, int retry, struct ptt_response *response, struct sockaddr_in pttaddr){
	...
	if (sockfd < 0) {
		ast_log(LOG_ERROR, "Unable to create network socket to pttserver: %s\n", strerror(errno));
		return 0;
	}
	tv.tv_sec = 0;
	tv.tv_usec = 300000;
	...
	while(retry){
		...
		sendto(sockfd, request, sizeof(struct ptt_request), 0, (struct sockaddr *)&pttaddr, sizeof(pttaddr));
		recv_num = recvfrom(sockfd, recvbuf, sizeof(recvbuf), 0, (struct sockaddr *)&pttaddr, &len);
		if(recv_num <= 0){
			if(retry <= 0){
				ast_log(LOG_DEBUG, "pttserver recvfrom error : %s\n", strerror(errno));
				break;
			}else
				continue;
		}else{
			memcpy(response, recvbuf, sizeof(struct ptt_response));
			break;
		}
	}
	close(sockfd);
