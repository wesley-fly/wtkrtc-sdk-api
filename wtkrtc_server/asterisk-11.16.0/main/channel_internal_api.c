...
struct ast_channel {
	...
	struct sockaddr_in relayip;
	struct sockaddr_in pttip;  
    char pttid[AST_MAX_CONTEXT];
    int business_mode;         //conference,ptt or other groups
    int isstatectl;
};
...
{
	memcpy(&chan->relayip, &value, sizeof(struct sockaddr_in));
}
