...
ASTERISK_FILE_VERSION(__FILE__, "$Revision: 431384 $")
...
static int __rtp_recvfrom(struct ast_rtp_instance *instance, void *buf, size_t size, int flags, struct ast_sockaddr *sa, int rtcp)
{
	...
	/*if(*(int *)buf == 0xfffe16){
		ast_log(LOG_WARNING, "BUF = %x, buf + 11 = %x\n",*(int *)buf,*(int *)(buf+11));*(int *)buf
		//ast_log(LOG_WARNING, "1>===buf0=%x , buf1=%x ,buf2=%x ,buf3=%x ,in=%d, ver=%d , from=%s\n",*buf[0],*buf[1],*buf[2],*buf[3],*in,ver,ast_sockaddr_stringify(sa));
		char *newsessionticket = buf+11;
		ast_log(LOG_WARNING, "newsessionticket = %d\n",*newsessionticket);

		if(*newsessionticket == 4){
	...
	/*if(*(int *)buf == 0x6c000100){
		return -1;
	}*/
	//ast_log(LOG_WARNING, "1>===buf=%x, in=%d, from=%s\n",*(int *)buf,*in,ast_sockaddr_stringify(sa));
...
static int __rtp_sendto(struct ast_rtp_instance *instance, void *buf, size_t size, int flags, struct ast_sockaddr *sa, int rtcp, int *ice, int use_srtp)
{
	...
	*ice = 0;
	...
	if (use_srtp && res_srtp && srtp && res_srtp->protect(srtp, &temp, &len, rtcp) < 0) {
		ast_log(LOG_WARNING, "__rtp_sendto error\n");
		return -1;
	}
	//ast_log(LOG_WARNING, "Debug %x,:%x, size=%d,%d\n",*(int *)buf,*(int *)temp, size,len);
	//ast_log(LOG_WARNING, "Debug2 %x,:%x, size=%d,%d\n",*(int *)buf+3,*(int *)temp+3, size,len);
...
#endif
	ast_log(LOG_WARNING, "1>===buf=%x, from=%s\n",*(int *)buf,ast_sockaddr_stringify(sa));
...
static int ast_rtp_raw_write(struct ast_rtp_instance *instance, struct ast_frame *frame, int codec)
{
	...
	if (frame->frametype == AST_FRAME_VOICE) {
		...
		if (ast_tvzero(frame->delivery)) {
			...
			if (frame->subclass.format.id == AST_FORMAT_OPUS) {
				if (abs(rtp->lastts - pred) < MAX_TIMESTAMP_SKEW*2) {//For huawei callcenter
					rtp->lastts = pred;
				} else {
		...
		if (ast_tvzero(frame->delivery)) {
			if (abs(rtp->lastts - pred) < 7200) {
				...
				ast_debug(3, "Video Difference is %d, ms is %u (%u), pred/ts/samples %u/%d/%d\n", abs(rtp->lastts - pred), ms, ms * 90, rtp->lastts, pred, frame->samples);
	...
	if (!ast_sockaddr_isnull(&remote_address)) {
		if (0)//frame->frametype == AST_FRAME_VOICE AST_FRAME_VOICE AST_FRAME_VIDEO
		{
			int res, ice;
			if ((res = rtp_sendto(instance, (void *)frame->data.ptr, frame->datalen, 0, &remote_address, &ice)) < 0) {
...
static int ast_rtp_write(struct ast_rtp_instance *instance, struct ast_frame *frame)
{
	...
	/*else if (frame->frametype == AST_FRAME_RTCP) {
		ast_log(LOG_WARNING, "res_rtp_asterisk, requested to send a RTCP PSFB packet to the peer\n");
	...
	if (rtp->smoother) {
		struct ast_frame *f;
		ast_log(LOG_WARNING, "res_rtp_asterisk, rtp->smoother\n");
		...
		//unsigned char *rtpheader = (unsigned char *)(frame->data.ptr);
		//ast_log(LOG_WARNING, "res_rtp_asterisk, offset=%d, 0=%x,1=%x rtp->smoother\n",frame->offset,rtpheader[0],rtpheader[1]);
...
static struct ast_frame *ast_rtp_read(struct ast_rtp_instance *instance, int rtcp)
{
	...
	if (rtp->strict_rtp_state == STRICT_RTP_LEARN) {
		ast_verb(4, "%p -- Probation learning mode pass with source address %s\n", rtp, ast_sockaddr_stringify(&addr));
		...
		if (rtp_learning_rtp_seq_update(&rtp->rtp_source_learn, seqno)) {
			ast_verb(4, "%p -- Probation at seq %d with %d to go; discarding frame\n",
