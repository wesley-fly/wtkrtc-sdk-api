...
#define AUTH_WHITELIST 1
#define AUTH_MAXCOUNT  2
#define AUTH_PWD       3

#define BUSI_PTT       1
#define BUSI_AUDIO_CNF 2
#define BUSI_VEDIO_CNF 3

#define PTT_TEMP_BAND 1
#define PTT_FOREVER_BAND 2
...
struct ptt_request{
	char action[6+1];
	char id[32+1];              //unique id ,bandid
	int BandMode;
	char CallerNumber[30+1];    //caller freepp
	char BandCallNumber[30+1];  //bandno or callee freepp
	char BandID[30+1];
	char BandName[50+1];
};
struct ptt_response{
...
struct sockaddr_in *ast_channel_relayip(const struct ast_channel *chan);
void ast_channel_relayip_set(struct ast_channel *chan, struct sockaddr_in value);
struct sockaddr_in *ast_channel_pttip(const struct ast_channel *chan);
const char *ast_channel_pttid(const struct ast_channel *chan);
void ast_channel_pttid_set(struct ast_channel *chan, const char *value);
void ast_channel_pttip_set(struct ast_channel *chan, struct sockaddr_in value);
int ast_channel_business_mode(const struct ast_channel *chan);
void ast_channel_business_mode_set(struct ast_channel *chan, int value);
int ast_channel_isstatectl(const struct ast_channel *chan);
void ast_channel_isstatectl_set(struct ast_channel *chan, int value);
int ast_get_ptt_response(struct ptt_request *request, int retry, struct ptt_response *response, struct sockaddr_in pttaddr);
