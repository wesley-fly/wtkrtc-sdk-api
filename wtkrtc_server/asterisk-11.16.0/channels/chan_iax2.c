...
ASTERISK_FILE_VERSION(__FILE__, "$Revision: 429673 $")
...
static char global_relay_server[32];
static int global_relay_port = 4579;
...
static iax2_format iax2_codec_choose(struct ast_codec_pref *pref, iax2_format formats, int find_best)
{
	...
	ast_verb(3, "iax2_codec_choose formats = %lx\n",formats);
...
static int iax2_call(struct ast_channel *c, const char *dest, int timeout)
{
	...
	iax_ie_append_str(&ied, IAX_IE_CALLED_NUMBER, pds.peer/*pds.exten*/);
...
static int iax2_start_transfer(unsigned short callno0, unsigned short callno1, int mediaonly)
{
	...
	//Xiaofan generate rs_token
	struct sockaddr_in relay_array[5];
	int x;
	...
	//char rsaddr[32] = "192.168.4.163";
	//int rsport = 4579;

	relay_array[0].sin_family = AF_INET;
	relay_array[0].sin_port = htons(global_relay_port);
	relay_array[0].sin_addr.s_addr = inet_addr(global_relay_server);
	if (1)
	{
		gettimeofday(&now, NULL);
		sprintf(token,"%s%s%d%d%ld%ld",iaxs[callno0]->cid_num,iaxs[callno0]->exten,callno0,callno1,now.tv_sec,now.tv_usec);
		MD5Init(&md5);
		MD5Update(&md5, (unsigned char *)token, strlen(token));
		MD5Final(digest, &md5);
		for (x=0;x<16;x++){
			sprintf(tokenmd5 + (x << 1), "%2.2x", digest[x]); /* safe */
		}
		ast_log(LOG_DEBUG,"-----generate relayserver tokenid = %s, tokenmd5=%s\n", token, tokenmd5);
	}

	memset(&ied0, 0, sizeof(ied0));
	iax_ie_append_addr(&ied0, IAX_IE_APPARENT_ADDR, /*&iaxs[callno1]->addr*/&relay_array[0]);
	iax_ie_append_short(&ied0, IAX_IE_CALLNO, iaxs[callno1]->peercallno);
	iax_ie_append_int(&ied0, IAX_IE_TRANSFERID, transferid);
	if (!ast_strlen_zero(tokenmd5))
		iax_ie_append_str(&ied0, IAX_IE_RELAY_TOKEN, tokenmd5);

	memset(&ied1, 0, sizeof(ied1));
	iax_ie_append_addr(&ied1, IAX_IE_APPARENT_ADDR, /*&iaxs[callno0]->addr*/&relay_array[0]);
	iax_ie_append_short(&ied1, IAX_IE_CALLNO, iaxs[callno0]->peercallno);
	iax_ie_append_int(&ied1, IAX_IE_TRANSFERID, transferid);
	if (!ast_strlen_zero(tokenmd5))
		iax_ie_append_str(&ied1, IAX_IE_RELAY_TOKEN, tokenmd5);
...
static int iax2_answer(struct ast_channel *c)
{
	...
	if(ast_channel_business_mode(c) == BUSI_PTT){
		ast_log(LOG_DEBUG,"ptt server : send pttip and pttid to caller in answer : %s:%d,%s\n",ast_inet_ntoa(ast_channel_pttip(c)->sin_addr), ntohs(ast_channel_pttip(c)->sin_port),ast_channel_pttid(c));
		iax_ie_append_addr(&ied, IAX_IE_PTTSERVER, ast_channel_pttip(c));
		iax_ie_append_str(&ied, IAX_IE_PTTSESSION, ast_channel_pttid(c));
		ie_flag = 1;
	}
	if((ast_channel_business_mode(c) == BUSI_AUDIO_CNF) || (ast_channel_business_mode(c) == BUSI_VEDIO_CNF)){
		ast_log(LOG_DEBUG, "statectl : send mixer addr to caller in answer : %s:%d\n",ast_inet_ntoa(ast_channel_relayip(c)->sin_addr), ntohs(ast_channel_relayip(c)->sin_port));
		iax_ie_append_addr(&ied, IAX_IE_MIXSERVER, ast_channel_relayip(c));
		
		ie_flag = 1;
	}
	if(ie_flag == 1){
		return send_command_locked(callno, AST_FRAME_CONTROL, AST_CONTROL_ANSWER, 0, ied.buf, ied.pos, -1);
	}else{
					...
					if (strcmp(iaxs[fr->callno]->exten, "TBD") && !exists) {
						...
						if (format) {
							...
							if (strcmp(iaxs[fr->callno]->exten, "TBD")) {
								...
								ast_verb(3, "AAAAccepting UNAUTHENTICATED call from %s:\n"
												"%srequested format = %s,\n"
												"%srequested prefs = %s,\n"
												"%sactual format = %s,\n"
												"%shost prefs = %s,\n"
...
static int set_config(const char *config_file, int reload, int forced)
{
	...
	while(v) {
		if (!strcasecmp(v->name, "bindport")){ 
			...
			if (maxauthreq < 0)
				maxauthreq = 0;
		} else if (!strcasecmp(v->name, "relayserver")) {
			ast_copy_string(global_relay_server, v->value, sizeof(global_relay_server));
		}	else if (!strcasecmp(v->name, "relayport")) {
		  global_relay_port = atoi(v->value);
		}	else if (!strcasecmp(v->name, "adsi")) {
